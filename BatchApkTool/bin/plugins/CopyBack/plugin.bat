:: CopyBack v1.0
:: Автор: bursoft
:: Плагин заменяет *.apk и *.jar файлы в папках _app, _priv-app и _framework на одноименные файлы из папок _OUT_APK и _OUT_JAR.
...
set count5=0
set progressbar=0
FOR /f "delims=" %%F IN ('sfk list -hidden -quiet -quot -dir _app _priv-app _framework -file .apk .jar 2^>nul') DO (set /A count2+=1)
FOR /f "delims=" %%a IN ('sfk list -hidden -quiet -quot -dir _app _priv-app _framework -file .apk .jar 2^>nul') DO (
